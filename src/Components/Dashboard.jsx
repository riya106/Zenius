...
const Dashboard = ({ onClickLogin, onOpenInternship, onOpenHackathon }) => {
  ...
  if (showLogin) {
    return <Login />; // show login page when clicked
  }
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center mt-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-16 text-center">
          {[
            { name: "Internship", color: "from-indigo-500 to-purple-500" },
            { name: "Hackathon", color: "from-purple-500 to-pink-500" },
            { name: "DSA", color: "from-pink-500 to-indigo-500" },
            { name: "Summits", color: "from-blue-500 to-violet-600" },
          ].map((item, index) => (
            <div
              key={index}
